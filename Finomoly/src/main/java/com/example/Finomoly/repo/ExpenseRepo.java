...
public interface ExpenseRepo extends JpaRepository<Expenses,Integer> {
    ...
    @Query("SELECT e.expenseCategory FROM Expenses e WHERE e.user.userMail = :userMail AND e.expenseAmount = (SELECT MAX(e2.expenseAmount) FROM Expenses e2 WHERE e2.user.userMail = :userMail)")
